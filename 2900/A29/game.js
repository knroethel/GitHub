...
const G = ( function () {
	...
	var COLOR_CANVAS = 0x3C2940;
	var COLOR_PAINT = 0x88EBEB;
	var COLOR_BLOCK = 0x417291;
	...
	// Variables to avoid repeating actions or sounds
	var clicks = 0;
	var victoy_play = 0;
	...
	const _LEVELS = [
		...
		{
			muse: "!",
			map: "images/level_EXCLAMATION.gif"
		},

		{
			muse: "→",
			map: "images/level_ARROW.gif"
		},

		{
			muse: "⌛",
			map: "images/level_HOURGLASS.gif"
		},

		{
			muse: "🕒",
			map: "images/level_CLOCK.gif"
		},

		{
			muse: "♣",
			map: "images/level_CLOVER.gif"
		},

		{
			muse: "GAME OVER",
			map: "images/level_GAMEOVER.gif"
		},
	]
	...
	// Changes color scheme of the level

	const color_change = function(){

		if ( _level === 0 ){
			COLOR_CANVAS = 0x3C2940;
			COLOR_PAINT = 0x88EBEB;
			COLOR_BLOCK = 0x417291;
		}

		else {
			if (_level === 1 ) {
				COLOR_CANVAS = 0x3E3E3E;
				COLOR_PAINT = 0xF5F4A6;
				COLOR_BLOCK = 0xA3A251;
			}

			else {
				if ( _level === 2 ){
					COLOR_CANVAS = 0xE9BAEF;
					COLOR_PAINT = 0xCFA081;
					COLOR_BLOCK = 0xA37659;
				}

				else {
					if ( _level === 3 ){
						COLOR_CANVAS = 0x7A81C2;
						COLOR_PAINT = 0xB3CEAA;
						COLOR_BLOCK = 0x8DA983;
					}

					else {
						if ( _level === 4 ){
							COLOR_CANVAS = 0xF1A6A6;
							COLOR_PAINT = 0x85B3CE;
							COLOR_BLOCK = 0x5987A3;
						}

						else {
							if ( _level === 5 ){
								COLOR_CANVAS = 0xFBD762;
								COLOR_PAINT = 0xF9F5E8;
								COLOR_BLOCK = 0xB2AFA4;
							}

							else {
								if ( _level === 6 ){
									COLOR_CANVAS = 0xDBDBDB;
									COLOR_PAINT = 0xE25454;
									COLOR_BLOCK = 0xAD3131;
								}

								else {
									if ( _level === 7 ){
										COLOR_CANVAS = 0xA1DBC8;
										COLOR_PAINT = 0xB7A384;
										COLOR_BLOCK = 0x917A55;
									}

									else {
										if ( _level === 8 ){
											COLOR_CANVAS = 0xC6D4D8;
											COLOR_PAINT = 0x868686;
											COLOR_BLOCK = 0x646464;
										}

										else {

											if ( _level === 9 ){
												COLOR_CANVAS = 0x777AAD;
												COLOR_PAINT = 0xB6CF7A;
												COLOR_BLOCK = 0x94B345;
											}

											else {
												if ( _level === 10 ){
													COLOR_CANVAS = 0x6A6671;
													COLOR_PAINT = 0xF19871;
													COLOR_BLOCK = 0xC06E49;
												}
											}
										}
									}
	...
	// Adds and removes hint, but stops the call if the status line is displaying important info
	
	const remove_hint = function (){
		let level = _LEVELS[ _level - 1 ];

		if ( PS.statusText() === "A Masterpiece! Any button to continue." || PS.statusText() === "You win!" ) {
			return;
		}
		else {
	...
	return {
		init : function () {
			...
			// Timer
			hint_time = PS.timerStart( 2100, give_hint );
