...
const G = ( function () {
			...
			for ( let i = 0; i < zone.length; i += 1 ) {
				...
				if ( ( dx >= 0 ) && ( dx < grid_x ) && ( dy >= 0 ) && ( dy < grid_y ) ) {
					if ( PS.alpha( dx, dy ) !== PS.ALPHA_TRANSPARENT ) {
						...
						if ( mazedata[ ( dy * grid_x ) + dx ] === MAP_TOME ) {
							place_tome( dx, dy );
						}
						// seen[ ( dy * grid_x ) + dx ] = PS.ALPHA_TRANSPARENT;
					}
				}
			}
		}
	...
	const collect_tome = function ( x, y ) {
		...
		PS.gridPlane( PLANE_TOME );
		PS.fade( x, y, 60, { rbg: PS.COLOR_WHITE } );
	...
	// Called to initiate ending
	const ending = function () {

		if ( ending_clicks === 1 ) {
			PS.statusText( "No, surely this can't be right..." );
		}

		if ( ending_clicks === 2 ) {
			PS.statusText( "After all this time and effort..." );
		}

		if ( ending_clicks === 3 ) {
			PS.statusText( "This is how we're rewarded?" );
		}

		if ( ending_clicks === 4 ) {
			PS.statusText( "All for a stupid little jo-" );
		}
		...
		if ( ending_clicks === 7 ) {
			PS.statusText( "My shoe WAS untied." );
		}

		if ( ending_clicks === 8 ) {
			PS.statusText( "How embarrassing." );
		}

		if ( ending_clicks === 9 ) {
			PS.statusText( "Just...forget any of this happened." );
		}
		...
		if ( ending_clicks === 10 ) {
			...
			PS.spriteShow( scholar_sprite, false );
			game_over = true;
		}
	...
	const on_load  = function ( image ) {
		...
		for ( let y = 0; y < grid_y; y += 1 ) {
			for ( let x = 0; x < grid_x; x += 1 ) {
				...
				switch ( pixel ) {
					...
					case MAP_TOME:
						tomes_total += 1;
						// place_tome ( x, y ); // illuminate() has to do this
						break;
					case MAP_SCHOLAR:
						place_scholar ( x, y );
						data = MAP_PATHWAY;
		...
		illuminate( scholar_x, scholar_y );
	...
	return {
		init: function () {
			...
			//Unmake path color
			rgb_pathway = PS.unmakeRGB( COLOR_PATHWAY, {} );
